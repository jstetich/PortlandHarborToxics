...
```{r}
tmp <- res_wide %>% select(starts_with('LN'))

cor(log(tmp), use='pairwise')

```

```{r}
res_long_MLE <- res_wide %>%
...
plt <- ggplot(res, aes(Contaminant, MLE)) + 
  #geom_boxplot() +
...
```{r}
ggplot(res, aes(as.numeric(Contaminant), MLE)) +
